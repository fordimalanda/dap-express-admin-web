...
export default function ProductsPage() {
  ...
  const handleCopyLink = (productSlug: string, source = "tiktok_ads") => {
    const link = `http://localhost:3000/p/${productSlug}?utm_source=${source}&utm_campaign=direct_sale`;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Catalogue Produits & Liens Publicitaires</h1>
          <p className="text-sm text-slate-500 mt-0.5">
            Gérez vos offres et générez des liens de tracking pour vos campagnes Ads (TikTok, Meta, Google)
          </p>
        </div>
            ...
            <div className="p-5 flex-1 flex flex-col justify-between">
              ...
              <div className="mt-5 pt-4 border-t border-slate-100 space-y-2">
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block">
                  Lien Campagne TikTok / Meta
                </span>
                <div className="flex gap-2">
                  <button
                    onClick={() => handleCopyLink(p.slug, "tiktok_ads")}
                  ...
                  >
                    {copiedSlug === p.slug ? (
                      ...
                      <>
                        <Copy className="h-3.5 w-3.5" /> Copier Lien Ads
                      </>
                    )}
                  </button>
                  <a
                    href={`http://localhost:3000/p/${p.slug}`}
                    target="_blank"
